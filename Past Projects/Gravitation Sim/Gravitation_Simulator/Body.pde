...
class Body {
    ...
    //converts the radius from meters to pixels given the value of meters per pixel
    float radius;
    ...
    void initialVelocity(PVector initialMousePosition, PVector finalMousePosition) {
        ...
        if (PVector.sub(initialMousePosition, finalMousePosition).mag()<0.01) {
            ...
            float initialVelocity = PVector.mult(PVector.sub(initialMousePosition, finalMousePosition),dt).mag();
            ...
            float scalingFactor = 30;
            //1/(3*sqrt(initialVelocity));
